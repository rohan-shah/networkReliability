...
for(i in 1:nrow(scenarios))
{
	...
	pdf(file.path("runningVariances", paste0(scenarios[i, "method"], "-", scenarios[i, "sampleSize"], "-", scenarios[i, "graph"], "-", scenarios[i, "probability"], ".pdf")))
		print(ggplot(data = data, mapping = aes(x, value, group = method, colour = method)) + geom_line() + scale_y_log10(breaks = scales::pretty_breaks(n = 10), labels = scientific_10) + ylab("Log10 of estimated variance") + xlab("Number of samples"))
	dev.off()
