...
formatTheme <- theme(axis.text.x = element_text(size = 22), axis.title.x = element_text(size = 24), axis.text.y = element_text(size = 22), axis.title.y = element_text(size = 20), legend.text = element_text(size = 18), legend.title = element_text(size = 18), legend.key.width = unit(3, "line"))
plot <- data %>% filter(graph == "dodecahedron" & probability == 0.99 & method != "approxZeroVarianceWORWithVariance" & method != "residualResampling") %>% ggplot(mapping = aes(sampleSize, workNormalizedVariance, colour = method, linetype = method)) + scale_y + geom_line(size = size) + scale_x_log10(breaks = c(10, 100, 1000, 10000), labels = c(10, 100, 1000, 10000)) + theme(legend.position = c(0.2, 0.2)) + formatTheme + xlab("Sample size") + ylab("WNV") + guides(colour = guide_legend(title = "Method"), linetype = guide_legend(title = "Method"))
pdf("./dodecahedronWNV0_99.pdf")
print(plot)
dev.off()
#Plot RE, for the dodecahedron with p = 0.99
plot <- data %>% filter(graph == "dodecahedron" & probability == 0.99 & method != "approxZeroVarianceWORWithVariance" & method != "residualResampling") %>% ggplot(mapping = aes(sampleSize, relativeError, colour = method, linetype = method)) + scale_y + geom_line(size = size) + scale_x_log10(breaks = c(10, 100, 1000, 10000), labels = c(10, 100, 1000, 10000)) + theme(legend.position = c(0.2, 0.2)) + formatTheme + xlab("Sample size") + ylab("RE") + guides(colour = guide_legend(title = "Method"), linetype = guide_legend(title = "Method"))
...
plot <- data %>% filter(graph == "dodecSeries" & probability == 0.9999 & method != "approxZeroVarianceWORWithVariance" & method != "residualResampling") %>% ggplot(mapping = aes(sampleSize, empiricalVariance, colour = method, linetype = method)) + scale_y + geom_line(size = size) + scale_x_log10(breaks = c(10, 100, 1000, 10000), labels = c(10, 100, 1000, 10000)) + theme(legend.position = c(0.2, 0.2)) + formatTheme + xlab("Sample size") + ylab("Var") + guides(colour = guide_legend(title = "Method"), linetype = guide_legend(title = "Method"))
