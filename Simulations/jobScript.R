...
} else if(method == "approxZeroVarianceWORMerge")
{
	counter <- 1
	if(file.exists(outputFile))
	{
	...
	while(counter < nReps + 1)
	{
		results[[counter]] <- approximateZeroVarianceWORMerge(graph = graph, probability = probability, n = sampleSize, seed = counter, interestVertices = interestVertices)
		if(counter %% 100 == 0)
		{
			save(results, file = tmpFile)
			file.rename(from = tmpFile, to = outputFile)
