...
produceTable <- function(indices, showEmpiricalMse)
{
	...
	retVal$Method <- c("approxZeroVarianceWOR" = "WOR", "approxZeroVariance" = "IS", "approxZeroVarianceWORMerge" = "WOR-Merge")[retVal$Method]
	return(retVal)
}
indices <- which(scenarios$graph == "dodecahedron" & scenarios$probability == 0.99 & !(scenarios$method %in% c("approxZeroVarianceWORWithVariance", "fearnhead")))
...
indices <- which(scenarios$graph == "grid9Augmented" & scenarios$probability == 0.9999 & !(scenarios$method %in% c("approxZeroVarianceWORWithVariance", "fearnhead")))
...
indices <- which(scenarios$graph == "dodecParallel" & scenarios$probability == 0.9999 & !(scenarios$method %in% c("approxZeroVarianceWORWithVariance", "fearnhead")))
...
indices <- which(scenarios$graph == "dodecSeries" & scenarios$probability == 0.9999 & !(scenarios$method %in% c("approxZeroVarianceWORWithVariance", "fearnhead")))
