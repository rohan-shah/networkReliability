...
namespace networkReliability
{
	...
	namespace subObs
	{
		...
		public:
			...
			withResampling(context const& contextObj, boost::shared_array<edgeState> state, double radius, int conditioningCount, mpfr_class conditiniongProb);
			int getMinCut() const;
		...
		private:
			withResampling(context const& contextObj, boost::shared_array<edgeState> state, double radius, ::networkReliability::subObs::withResamplingConstructorType&);
			withResampling(context const& contextObj, boost::shared_array<edgeState> state, double radius);
			void initialise();
