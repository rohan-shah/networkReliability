...
#ifndef BOOST_GRAPH_RECURSIVE_DFS_BINARY_HPP
#define BOOST_GRAPH_RECURSIVE_DFS_BINARY_HPP
...
namespace boost {
  namespace detail {
	template<class IncidenceGraph> struct depth_first_visit_fixed_impl_helper
	{
    ...
    void depth_first_visit_fixed_impl
      (const IncidenceGraph& g,
       typename graph_traits<IncidenceGraph>::vertex_descriptor u,
       DFSVisitor& vis,
       ColorMap color, typename depth_first_visit_fixed_impl_helper<IncidenceGraph>::stackType& stack, 
	   const int* state, TerminatorFunc func = TerminatorFunc())
    {
      ...
      while (!stack.empty()) {
        ...
        while (ei != ei_end) {
		  //Should we consider this edge?
			if(state[2 * boost::get(boost::edge_index, g, *ei)] > 1)
		  {
                     ...
                     const networkReliability::edgeState* state)
  {
		...
		for (boost::tie(ui, ui_end) = vertices(g); ui != ui_end; ++ui) {
			...
			{       
				...
				detail::depth_first_visit_fixed_impl(g, u, vis, color, stack, state, detail::nontruth2());
			}
		}
	}
