...
{
	namespace approximateZeroVarianceFearnheadImpl
	{
		struct approximateZeroVarianceFearnheadScratch
		{
	...
	void approximateZeroVarianceFearnhead(approximateZeroVarianceFearnheadArgs& args)
	{
		...
		sampling::fearnheadSamplingArgs samplingArgs;
		samplingArgs.n = n;
		std::vector<int>& indices = samplingArgs.indices;
		//Temporaries for calculating max flow values
		approximateZeroVarianceFearnheadImpl::approximateZeroVarianceFearnheadScratch scratch;
		...
		for(int edgeCounter = 1; edgeCounter < (int)nEdges; edgeCounter++)
		{
			...
			{
				indices.clear();
				sampling::fearnheadSampling(samplingArgs, args.randomSource);
				mpfr_class cInverse = 1.0 / samplingArgs.c;
				int counter = 0;
				for(std::vector<int>::iterator i = indices.begin(); i != indices.end(); i++)
				{
					...
					if(choices[*i].edgePresent)
					{
						...
						newWeight = weights[choices[*i].parentIndex]*opProbability;
					}
					else
					{
						newStates[counter*2*nEdges + 2*edgeCounter] = newStates[counter*2*nEdges + 2*edgeCounter + 1] = 0;
						newWeight = weights[choices[*i].parentIndex]*inopProbability;
					}
					if(samplingArgs.deterministicInclusion[*i])
					{
					...
					{
						newWeights.push_back(newWeight / (samplingArgs.c*newImportanceDensity[*i]));
						importanceDensity.push_back(cInverse);
					}
